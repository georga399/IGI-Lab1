...
def personal_cabinet(request):
    ...
    paid_reservations = reservations.filter(status='paid')
    reservations = reservations.filter(status='reserved')
...
def cart_payment(request):
    if request.method == 'POST':
        reservations = Reservation.objects.filter(user=request.user, status='reserved')
