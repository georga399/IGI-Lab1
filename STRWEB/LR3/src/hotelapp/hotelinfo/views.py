...
from .models import Company, Article, FAQ, Job, Partner, Promo, Review
from .forms import ReviewForm
from accounts.models import SliderSettings
...
def general(request):
    ...
    if request.method == "POST":
        new_interval = request.POST.get("interval")
        settings.interval = int(new_interval)
    ...
    return render(request, 'general.html',{'article': article, 'partners':partners, 'settings':settings})
...
def carES6(request):
    return render(request, 'carES6.html')

def carProto(request):
    return render(request, 'carProto.html')
